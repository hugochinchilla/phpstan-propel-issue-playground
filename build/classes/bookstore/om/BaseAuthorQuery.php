 ...
 *
 * @method AuthorQuery leftJoinAuthorKV($relationAlias = null) Adds a LEFT JOIN clause to the query using the AuthorKV relation
 * @method AuthorQuery rightJoinAuthorKV($relationAlias = null) Adds a RIGHT JOIN clause to the query using the AuthorKV relation
 * @method AuthorQuery innerJoinAuthorKV($relationAlias = null) Adds a INNER JOIN clause to the query using the AuthorKV relation
 *
...
abstract class BaseAuthorQuery extends ModelCriteria
{
    ...
    /**
     * Filter the query by a related AuthorKV object
     *
     * @param   AuthorKV|PropelObjectCollection $authorKV  the related object to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return   AuthorQuery The current query, for fluid interface
     * @throws   PropelException - if the provided filter is invalid.
     */
    public function filterByAuthorKV($authorKV, $comparison = null)
    {
        if ($authorKV instanceof AuthorKV) {
            return $this
                ->addUsingAlias(AuthorPeer::ID, $authorKV->getAuthorId(), $comparison);
        } elseif ($authorKV instanceof PropelObjectCollection) {
            return $this
                ->useAuthorKVQuery()
                ->filterByPrimaryKeys($authorKV->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByAuthorKV() only accepts arguments of type AuthorKV or PropelCollection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the AuthorKV relation
     *
     * @param     string $relationAlias optional alias for the relation
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return AuthorQuery The current query, for fluid interface
     */
    public function joinAuthorKV($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('AuthorKV');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'AuthorKV');
        }
    ...
    /**
     * Use the AuthorKV relation AuthorKV object
     *
     * @see       useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   AuthorKVQuery A secondary query class using the current class as primary query
     */
    public function useAuthorKVQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinAuthorKV($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'AuthorKV', 'AuthorKVQuery');
    }
