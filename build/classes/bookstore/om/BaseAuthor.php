...
abstract class BaseAuthor extends BaseObject implements Persistent
{
    ...
    /**
     * @var        AuthorKV one-to-one related AuthorKV object
     */
    protected $singleAuthorKV;
     ...
     * @var		PropelObjectCollection
     */
    protected $authorKVsScheduledForDeletion = null;
    ...
    protected function doSave(PropelPDO $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->authorKVsScheduledForDeletion !== null) {
                if (!$this->authorKVsScheduledForDeletion->isEmpty()) {
                    AuthorKVQuery::create()
                        ->filterByPrimaryKeys($this->authorKVsScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->authorKVsScheduledForDeletion = null;
                }
            }

            if ($this->singleAuthorKV !== null) {
                if (!$this->singleAuthorKV->isDeleted()) {
                        $affectedRows += $this->singleAuthorKV->save($con);
                }
            }
    ...
    protected function doValidate($columns = null)
    {
        if (!$this->alreadyInValidation) {
            ...
            if (($retval = AuthorPeer::doValidate($this, $columns)) !== true) {
                ...
                if ($this->singleAuthorKV !== null) {
                    if (!$this->singleAuthorKV->validate($columns)) {
                        $failureMap = array_merge($failureMap, $this->singleAuthorKV->getValidationFailures());
                    }
                }
    ...
    public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->singleAuthorKV) {
                $result['AuthorKV'] = $this->singleAuthorKV->toArray($keyType, $includeLazyLoadColumns, $alreadyDumpedObjects, true);
            }
        }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy && !$this->startCopy) {
            ...
            $relObj = $this->getAuthorKV();
            if ($relObj) {
                $copyObj->setAuthorKV($relObj->copy($deepCopy));
            }
    ...
    /**
     * Gets a single AuthorKV object, which is related to this object by a one-to-one relationship.
     *
     * @param PropelPDO $con optional connection object
     * @return AuthorKV
     * @throws PropelException
     */
    public function getAuthorKV(PropelPDO $con = null)
    {

        if ($this->singleAuthorKV === null && !$this->isNew()) {
            $this->singleAuthorKV = AuthorKVQuery::create()->findPk($this->getPrimaryKey(), $con);
        }

        return $this->singleAuthorKV;
    }

    /**
     * Sets a single AuthorKV object as related to this object by a one-to-one relationship.
     *
     * @param             AuthorKV $v AuthorKV
     * @return Author The current object (for fluent API support)
     * @throws PropelException
     */
    public function setAuthorKV(AuthorKV $v = null)
    {
        $this->singleAuthorKV = $v;

        // Make sure that that the passed-in AuthorKV isn't already associated with this object
        if ($v !== null && $v->getAuthor() === null) {
            $v->setAuthor($this);
        }
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep) {
            ...
            if ($this->singleAuthorKV) {
                $this->singleAuthorKV->clearAllReferences($deep);
            }
        } // if ($deep)
        ...
        $this->collBooks = null;
        if ($this->singleAuthorKV instanceof PropelCollection) {
            $this->singleAuthorKV->clearIterator();
        }
        $this->singleAuthorKV = null;
    }
